...
describe('Statistics View - Tracking Area', () => {
    it('should show all entries', () => {
        ...
        cy.contains('header', 'Tempo-Tracker').should('be.visible')
            .contains('a', 'Statistics').should('be.exist').click()

        cy.contains('div[style]', '40:00').should('exist')
            .find('span[style]').should('not.exist')
        cy.contains('div[style]', '40:00').should('exist')
            .contains('legend', '41').should('exist')
        cy.contains('div', 'Year')
            .find('input').should('have.value', '2020')


        cy.contains('div', 'Total Hours')
            .find('p').should('contain.text', '40h 00m')
        cy.contains('div', 'Required Hours')
            .find('p').should('contain.text', '40h 00m')
        cy.contains('div', 'Median Hours (Week)')
            .find('p').should('contain.text', '40h 00m')
        cy.contains('div', 'Overhours')
            .find('p').should('contain.text', '—')

        cy.contains('div', 'Hours per Week')
            .find('input').should('have.value', '40')
            .type('{selectall}35')
        ...
        cy.contains('div', 'Total Hours')
            .find('p').should('contain.text', '40h 00m')
        cy.contains('div', 'Required Hours')
            .find('p').should('contain.text', '35h 00m')
        cy.contains('div', 'Median Hours (Week)')
            .find('p').should('contain.text', '40h 00m')
        cy.contains('div', 'Overhours')
            .find('p').should('contain.text', '5h 00m')
        ...
        cy.contains('h6', 'Weekly Hours').should('exist')
            .contains('a', 'Refresh').should('exist')
        ...
        cy.contains('div', 'Total Hours')
            .find('p').should('contain.text', '45h 00m')
        cy.contains('div', 'Required Hours')
            .find('p').should('contain.text', '35h 00m')
        cy.contains('div', 'Median Hours (Week)')
            .find('p').should('contain.text', '40h 00m')
