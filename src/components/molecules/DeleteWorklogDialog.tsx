    ...
    return (
        <Modal style={{ width: 400, minHeight: 180, height: 'unset' }}>
            ...
            <DefaultText style={{ textAlign: 'center', marginBottom: 16 }}>
                {
                    `Do you really want to delete the worklog from ${timeString(log.start)}` +
                    ` till ${timeString(log.end)} for Ticket "${log.issue.name}".`
                }
            </DefaultText>
            <ButtonBar>
                <Button onClick={onClose}>Cancel</Button>
                {!!log.id && !log.synced && (
                    <Button onClick={() => { onClose(); onDelete(true) }}>
