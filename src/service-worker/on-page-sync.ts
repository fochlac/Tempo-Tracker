...
export async function reserveWorklog(log: TemporaryWorklog, tabId: number) {
    ...
    try {
        ...
        await DB.update(DB_KEYS.UPDATE_QUEUE, (queue) => queue.map((log) => (isThisWorklog(log) ? { ...entry, syncTabId: tabId } : log)))
    } finally {
        isReserving = false
    }
}
...
export async function unreserveWorklog(log: TemporaryWorklog, tabId: number) {
    ...
    try {
        ...
        await DB.update(DB_KEYS.UPDATE_QUEUE, (queue) => queue.map((log) => (isThisWorklog(log) ? { ...entry, syncTabId: null } : log)))
    } finally {
        isReserving = false
    }
}
